            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0" id="page-title">Conflict Types</h4>
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="javascript: void(0);">CRM</a></li>
                                <li class="breadcrumb-item active">Conflict Types</li>
                            </ol>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                            ...
                            <div class="card-header">
                                <div class="d-flex align-items-center flex-wrap gap-2">
                                    <div class="flex-grow-1">
                                        ...
                                        <a href="{{route('admin.conflict-types.index')}}" class="btn btn-info btn-sm add-btn">
                                ...
                                <table style="width: 100%;" id="buttons-datatables"
                                    ...
                                    @foreach($conflictTypes as $conflictType)
                                        <tr class="even">
                                            ...
                                            <td>{{$conflictType->name}}</td>
                                            <td>{{$conflictType->description}}</td>
                                            <td>
                                                <!-- Edit Button -->
                                                <a href="javascript:void(0);" class="edit-button btn btn-sm btn-primary" data-name="{{ $conflictType->name }}" data-slug="{{ $conflictType->slug }}" data-description="{{ $conflictType->description }}" title="Edit">
                    ...
                    <div class="col-xxl-3">
                        <div class="card border card-border-light">
                            <div class="card-header">
                                <h6 id="card-title" class="card-title mb-0">Add  Conflict Types</h6>
                            </div>
                            <div class="card-body">
                                <form id="edit-form" action="{{route('admin.conflict-types.store')}}" method="post" enctype="multipart/form-data">
                                    <input type="hidden" name="_method" value="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Conflict Types</label>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.edit-button').on('click', function() {
                ...
                $('#edit-form').attr('action', '/admin/conflict-types/' + slug + '/update');
                $('input[name="_method"]').val('PATCH');
                submitButton.text('Update');
                // Populate the form for editing
                $('#name').val(name);
                $('#card-title').text('Edit - ' + name + ' Conflict Types');
                $('#page-title').text('Edit - ' + name + ' Conflict Types');
            });
            ...
            $('#new-button').on('click', function() {
                ...
                submitButton.text('Add New');
                $('#name').val('');
                $('#card-title').text('Add Conflict Types');
                $('#page-title').text('Add New Conflict Types');
            });
        });
